...
/*void gobject:: drawingGame(float windowWidth,float windowHeight){
    for (int i=0;i<12000;i=i+1115)
            {
            set_pos(i,0); //drawing background
            window.draw(GetbckgSprite());
            }
        for (int i=0;i<12000;i=i+85){
            //if (i=255){continue;}
            set_pos(i, windowHeight-60); // drawing base ground
            window.draw(GetbaseSprite());
            }
        for (int i=1500;i<12000;i=i+1500){
            set_pos(i, windowHeight-122); // drawing check flag
            window.draw(GetcheckpointSprite());
            }
}*/
